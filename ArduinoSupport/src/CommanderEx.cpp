...
//#define DEBUG_SOCKET
 uint8_t Commander::s_debugLevel = 0;
...
void *Commander::XBeeThreadProc(void *pv)
{
    ...
    while(!pcmdr->_fCancel)
    {
        ...
        if(FD_ISSET(pcmdr->fdXBee, &fdset)) { 
           ...
           if (Commander::s_debugLevel ==2) printf("!");
            while((!pcmdr->_fCancel) && (ret = _read(pcmdr->fdXBee, &ch, 1)) > 0)
            {
            	if (Commander::s_debugLevel ==2) printf("%2x ", ch);
                ...
                else
                {
                    ...
                    if(pcmdr->index == 7)                 // packet complete
                    {
                        if(pcmdr->checksum%256 == 255)
                        {
                        	...
                        	if (Commander::s_debugLevel == 2) printf(" ERR\n");
...
void Commander::setDebugLevel(uint8_t bLevel)
{
	s_debugLevel = bLevel;
}
...
int Commander::ReadMsgs()
{
    if (fValidPacket)
    {
        ...
        if (s_debugLevel) {
        	printf("B:%x L:%d %d R:%d %d\n\r", buttons, leftV, leftH, rightV, rightH);
        }
