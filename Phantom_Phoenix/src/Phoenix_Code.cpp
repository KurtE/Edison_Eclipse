...
void LegIK (float feetPosX, float feetPosY, float feetPosZ, int legIndex)
{
    ...
    IKA14 = atan2f((float)(feetPosXZ-cCoxaLength[legIndex]-TarsOffsetXZ), (float)(feetPosY-TarsOffsetY));
...
boolean TerminalMonitor(void)
{
    ...
    if ((ich = DBGSerial.available()))
    {
        ...
        {
            if (!g_ServoDriver.ProcessTerminalCommand(szCmdLine, ich))
            	g_InputController.ProcessTerminalCommand(szCmdLine, ich);
        }
